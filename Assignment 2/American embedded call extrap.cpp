...
int main()
{
	...
	int iter_max{ 10 };  // maximum iterations
	...
	while(time < 2) {
		...
		// get option value
		double value = crank_nicolson(T, F, R, r, kappa, mu, S0, X, C, alpha, beta, sigma, i_max, j_max, S_max, rho, tol, iter_max, Cp, t0);
		...
		// extrap
		double Ex = (4. * value - value_old) / 3.;

		std::cout << i_max << "," << std::setprecision(10) << Ex  << "," << time << std::endl;

		V_store.push_back(Ex);
		n_store.push_back(i_max);
		t_store.push_back(time);
	...
	// open the csv file
	output.open("american vary step.csv");
	...
	if (output.is_open()) {
		...
		for (int i{ 0 }; i < V_store.size(); i++) {
			...
			output << n_store[i] << "," << V_store[i] << "," << t_store[i] << std::endl;
		}

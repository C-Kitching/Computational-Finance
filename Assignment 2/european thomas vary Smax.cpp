...
int main()
{
	...
	// set inital S_max
	double S_max_old = 5*F;

	// open a file stream for writing
	std::ofstream output;

	// open the csv file
	output.open("european thomas Smax.csv");

	// if the file is open
	if (output.is_open()) {

		// change Smax loop
		for (double S_max{ S_max_old }; S_max < 50 * F; S_max += F) {

			// scale imax and jmax
			i_max *= S_max / S_max_old;
			j_max *= S_max / S_max_old;

			auto start = std::chrono::steady_clock::now();  // get start time

			// get option value
			double option_value = crank_nicolson(T, F, R, r, kappa, mu, S0, X, C, alpha, beta, sigma, i_max, j_max, S_max);
			...
			// output result
			output << S_max/X << "," << option_value << "," << time << std::endl;
			//std::cout << S_max / F << "," << option_value << "," << time << std::endl;

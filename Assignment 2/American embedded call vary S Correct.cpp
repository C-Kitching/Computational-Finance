...
int main()
{
	...
	for (double S{ 0 }; S <= S_max; S += S_max / n) {
		S_store.push_back(S);
		V_store.push_back(crank_nicolson(T, F, R, r, kappa, mu, S, X, C, alpha, beta, sigma, i_max, j_max, S_max, rho, tol, iter_max, Cp, t0));
	}
	...
	// open the csv file
	output.open("American embedded call vary S Correct.csv");
	...
	if (output.is_open()) {
		...
		for (int i{ 0 }; i < S_store.size(); i++) {
			output << S_store[i] << "," << V_store[i] << "," << std::endl;
		}
	...
	const double& rho, const double& tol, const int& iter_max, const double& Cp, const double& t0)
{
	...
	for (int i{ i_max - 1 }; i >= 0; i--) {
		...
		// penalty method
		for (int penalty_itr{ 0 }; penalty_itr < iter_max; penalty_itr++) {
			...
			for (int j{ 1 }; j < j_max; j++) {
				...
				if (v_new[j] < R * S[j]) {
					b_hat[j] = b[j] - 0.5*rho;
					d_hat[j] = d[j] + rho * (0.5 * v_old[j] - R * S[j]);
				}
				...
				if (i * dt <= t0) {
					...
					if (v_new[j] > std::max(Cp, R * S[j])) {
						b_hat[j] = b[j] + 0.5 * rho;
						d_hat[j] = d[j] + rho * (std::max(Cp, R * S[j]) - 0.5 * v_old[j]);
					}
				}
			}

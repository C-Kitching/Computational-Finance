...
int main()
{
	...
	for (int N{ 1000 }; N <= 500000; N += 10000) {
		...
		// loop over the number of monte carlo paths
		for (int i{ 0 }; i < M; i++) {
			...
			if (i == 0) {
				...
				//portfolio_values.push_back(samples[0]);
			}
		}
		...
		portfolio_values.push_back(pop_mean);
		...
		upper_confidence.push_back(upper_95);
		lower_confidence.push_back(lower_95);
	}
	...
	// open the csv file
	output.open("confidence_intervals_S=X1.csv");
	...
	if (output.is_open()) {
		...
		for (int i{ 0 }; i < monte_carlo_N.size(); i++) {
			...
			// write data to file
			output << monte_carlo_N[i] << "," << portfolio_values[i] << "," << upper_confidence[i] << "," << lower_confidence[i] << std::endl;
		}

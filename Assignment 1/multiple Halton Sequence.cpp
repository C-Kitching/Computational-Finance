...
int main()
{
	...
	for (int N{ 1000 }; N <= 500000; N += 1000) {
		...
		portfolio_value.push_back(MonteCarlo(initial_share_price, interest_rate, dividend_rate, volatility, expiration, N, put_number, call_number, binary_put_number,
	...
	if (output.is_open()) {
		...
		for (int i{ 0 }; i < monte_carlo_N.size(); i++) {
			...
			// write data to file
			output << monte_carlo_N[i] << "," << portfolio_value[i] << std::endl;
		}
